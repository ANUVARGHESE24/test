...
class AccountMoveLine(models.Model):
    ...
    department_id = fields.Many2one(
        ...
        help="Department that will be used in payments, "
        ...
        for vals in vals_list:
            if vals.get("move_id", False):
                ...
                if move.department_id:
                    vals["department_id"] = move.department_id.id
    ...
    @api.constrains("department_id", "move_id")
    def _check_move_department_id(self):
        for rec in self:
            if (
                    rec.move_id
                    and rec.move_id.department_id
                    and rec.department_id
                    and rec.move_id.department_id != rec.department_id
            ):
                raise UserError(
                    _(
                        "Configuration error. The Department Id in"
                        " the Move Line and in the Move must be the"
                        " same."
                    )
                )
