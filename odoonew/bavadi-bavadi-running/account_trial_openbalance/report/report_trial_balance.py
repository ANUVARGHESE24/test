...
class ReportTrialBalance(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        accounts = docs if self.model == 'account.account' else self.env[
            'account.account'].search([])
        accountent = self.with_context(
            data['form'].get('used_context'))._get_accounts(accounts,display_account, operating_unit, department_id)
        #res = super(ReportTrialBalance, self)._get_report_values(self, docids, data=None)
        return {
            'doc_ids': docids,
            'doc_model': self.model,
            'data': data['form'],
