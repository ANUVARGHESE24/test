...
class AccountMove(models.Model):
    ...
    def _remove_delivery_line(self):
        self.env['account.move.line'].search([('move_id', 'in', self.ids), ('name', '=', 'Ship Delivery Charges')]).unlink()
    ...
    def _remove_transport_delivery_line(self):
        self.env['account.move.line'].search([('move_id', 'in', self.ids), ('name', '=', 'Transport Delivery Charges')]).unlink()
    ...
    def action_open_delivery_wizard_ship(self):
        view_id = self.env.ref('account_add_charges.account_choose_delivery_carrier_view_form').id
    ...
    def action_open_delivery_wizard_transport(self):
        view_id = self.env.ref('account_add_charges.account_choose_transport_delivery_carrier_view_form').id
        if self.env.context.get('carrier_recompute'):
            name = _('Update shipping cost')
            carrier = self.carrier_id
        else:
