...
class CashBookWizard(models.TransientModel):
    ...
    def check_report(self):
        ...
        if not self.account_ids:
            raise UserError(_("There is no  Cash account Ids Ids"))
        data = {}
    ...
    def get_xlsx_report(self):
        ...
        if self.initial_balance and not self.date_from:
            raise UserError(_("You must choose a Start Date"))
        if not self.account_ids:
            raise UserError(_("There is no Cash account Ids"))
        data = {}
        data['ids'] = self.env.context.get('active_ids', [])
        data['model'] = self.env.context.get('active_model', 'ir.ui.menu')
        data['form'] = self.read(
        ...
        data['form']['used_context'] = dict(used_context,
                                            lang=self.env.context.get(
                                                'lang') or 'en_US')
        return self.env.ref(
            'base_accounting_kit.action_report_cash_book_excel').report_action(self,
                                                                               data=data)
