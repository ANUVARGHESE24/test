...
class FinancialReport(models.TransientModel):
    ...
    def view_report_pdf(self):
            ...
            if not rec['parent']:
                ...
                for line in report_lines:
                    ...
                    if line[key] == rec['parent']:
                        return level + (set_report_level(line) or 0)

        # logging.info("****************report_lines %s" % report_lines)
        # finding the root
        ...
        return self.env.ref(
            'base_accounting_kit.expenses_report_pdf').with_context(
            landscape=True).report_action(self, data)
    ...
    def get_xlsx_report(self):
        ...
        def set_report_level(rec):
            ...
            if not rec['parent']:
                ...
                for line in report_lines:
                    key = 'a_id' if line['type'] == 'account' else 'id'
                    if line[key] == rec['parent']:
                        logging.info("set_report_level(line) %s %s" % (line[key], rec['parent']))
                        return level + (set_report_level(line) or 0)
        ...
        data['report_lines'] = report_lines
        # checking view type
        self.env.ref('base_accounting_kit.expenses_report_excel').report_file = "Expenses_Report"
        return self.env.ref(
            'base_accounting_kit.expenses_report_excel').report_action(self, data)
        ...
        if accounts:
            ...
            if operating_unit and not department_id:
                ...
                request = ("SELECT account_id as id, " +
                           ...
                           " WHERE account_id IN %s " +
                           filters +
                           "AND account_move_line.department_id=" + str(department_id) +
                           " GROUP BY account_id")
    ...
    def get_account_lines(self, data):
        ...
        logging.info("get_account_lines%s" % account_report)
        ...
        for report in child_reports:
            ...
            if res[report.id].get('account'):
                ...
                for account_id, value \
                    ...
                    # logging.info("%%%%%%%%%%%%%%%% %s %s" % (account.name, account.code))
                    vals = {
        ...
        # logging.info("ppppppppppppppppppppppppppp %s %s" % (lines, vals))
        return lines
    ...
    def find_journal_items(self, report_lines, form):
        ...
        journal_items = []
        # logging.info("**********************************%s" % form)
        # logging.info("**********************************%s" % form['operating_unit'])

        operating_unit = str(form['operating_unit'])
        department_id = str(form['department_id'])
        for i in report_lines:
            if i['type'] == 'account':
                account = i['account']
                if form['target_move'] == 'posted':
                    if form['operating_unit'] and not form['department_id']:
                        logging.info("operating_unit is called.............")
                        search_query = "select aml.id, am.id as j_id, aml.account_id, aml.date," \
                                       ...
                                       + " where aml.account_id=%s and aml.operating_unit_id= " + operating_unit
                        vals = [form['target_move']]
                        # logging.info("**********elif condion")
                    elif form['department_id'] and not form['operating_unit']:
                        logging.info("operating_unit is not called.............")
                        search_query = "select aml.id, am.id as j_id, aml.account_id, aml.date," \
                                       " aml.name as label, am.name, " \
                                       + "(aml.debit-aml.credit) as balance, aml.debit, aml.credit, aml.partner_id " \
                                       + " from account_move_line aml join account_move am " \
                                         "on (aml.move_id=am.id and am.state=%s) " \
                                       + " where aml.account_id=%s and aml.department_id= " + department_id
                        vals = [form['target_move']]
                        # logging.info("**********elif condion")
                    elif form['department_id'] and form['operating_unit']:
                        logging.info("operating_unit and department is  called.............")
                        search_query = "select aml.id, am.id as j_id, aml.account_id, aml.date," \
                                       " aml.name as label, am.name, " \
                                       + "(aml.debit-aml.credit) as balance, aml.debit, aml.credit, aml.partner_id " \
                                       + " from account_move_line aml join account_move am " \
                                         "on (aml.move_id=am.id and am.state=%s) " \
                                       + " where aml.account_id=%s and aml.department_id= " + department_id \
                                       + " and aml.operating_unit_id= " + operating_unit
                        vals = [form['target_move']]
                    else:
                        search_query = "select aml.id, am.id as j_id, aml.account_id, aml.date," \
                                       " aml.name as label, am.name, " \
...
class ProfitLossPdf(models.AbstractModel):
    ...
    _name = 'report.base_accounting_kit.report_expenses'
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        sheet.merge_range('A2:F2', data['form']['account_report_id'][1], head)
        ...
        if data['form']['debit_credit'] == 1:
            ...
            # logging.info("data['report_lines'] %s" % data['report_lines'])
            for a in data['report_lines']:

                if a['level'] != 0:
                    # sheet.write(row_num + 1, col_num, str('..' * a.get('level', 0)) + " " + str(a.get('name')),
                    #             txt_left)
                    if a.get('account_name', 'nothing') == 'nothing':
                        sheet.write(row_num + 1, col_num, '',
                                    txt_left)
                    else:
                        sheet.write(row_num + 1, col_num, a.get('code'),
                                    txt)

                    if a.get('account_name', 'nothing') == 'nothing':
                        sheet.write(row_num + 1, col_num + 1, a.get('name'),
                                    txt_left)
                    else:
                        sheet.write(row_num + 1, col_num + 1, a.get('account_name'),
                                    txt)
                    sheet.write(row_num + 1, col_num + 2, str(round(a.get('debit'), 2)),
                                txt_left)
                    sheet.write(row_num + 1, col_num + 3, str(round(a.get('credit'), 2)),
                                txt_left)
                    sheet.write(row_num + 1, col_num + 4, str(round(a.get('balance'), 2)),
                                txt_left)
                    # total_debit += a.get('debit') if a.get('debit') else 0.0
                    # logging.info("total_debit %s" % total_debit)
                    # total_credit += a.get('credit') if a.get('credit') else 0.0
                    row_num = row_num + 1
            # sheet.write(row_num + 1, col_num, "Total", bold)
            # sheet.write(row_num + 1, col_num + 2, str(round(total_debit, 2)),
            #             bold)
            # sheet.write(row_num + 1, col_num + 3, str(round(total_credit, 2)), bold)
            # sheet.write(row_num + 1, col_num + 4,
            #             str(round(total_debit - total_credit, 2)),
            #             bold)
        if not data['form']['enable_filter'] and not data['form']['debit_credit']:
            sheet.write('A9', 'Code', txt)
            sheet.write('B9', 'Account', txt)
            sheet.write('C9', 'Balance (SR)', bold)
            for a in data['report_lines']:
                if a['level'] != 0:
                    if a.get('account_name', 'nothing') == 'nothing':
                        sheet.write(row_num + 1, col_num, '',
                        ...
                        sheet.write(row_num + 1, col_num, a.get('code'),
                                    txt)
                    if a.get('account_name', 'nothing') == 'nothing':
                        sheet.write(row_num + 1, col_num + 1, a.get('account_name'), txt_left)
                    else:
                        sheet.write(row_num + 1, col_num + 1, a.get('name'), txt)
                    sheet.write(row_num + 1, col_num + 2, str(round(a.get('balance'), 2)),
                                txt_left)
                    # total_balance += a.get('balance') if a.get('balance') else 0.0
                    row_num = row_num + 1
