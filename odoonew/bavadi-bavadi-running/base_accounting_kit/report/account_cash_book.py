...
class ReportCashBook(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        accounts = self.env['account.account'].search(
            [('id', 'in', account_ids)])
        # logging.info("account ids %s %s" % (account_ids, accounts))
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        sheet = workbook.add_worksheet("Cash Book Report")
        ...
        sheet.merge_range('A2:H2', 'Cash Book Report', head)
        sheet.write('A5', 'Journal:', bold)
        if print_journal:
            sheet.write('B5:H5', ', '.join([lt or '' for lt in print_journal]), txt)
        ...
        if accounts_res:
            for i in accounts_res:
                ...
                for line in i['move_lines']:
                    ...
                    #     sheet.write(row_num + 1, col_num + 9, line['amount_currency'], txt_left)
                row_num = row_num + 1
            row_num = row_num + 1
