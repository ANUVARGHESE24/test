...
class AccountWizard(models.TransientModel):
    ...
    def get_xlsx_report(self, data, response):
        ...
        txt_center = workbook.add_format({'align': 'center',
                                          'font_size': '10px',
                                          'border': 1})
        pk = workbook.add_format({'align': 'center', 'font_size': 10, 'bg_color': '#D3D3D3', 'border': 1, 'bold': True})
        ...
        # sheet.set_column('B:C', 20, cell_format)
        # sheet.set_column('D:D', 20, cell_format)
        sheet.write('A2', "Report Date", txt)
        sheet.write('B2', str(data['today']), txt)
        sheet.write('D2', logged_users.name, txt)
        # sheet.merge_range('A3:D5', '')
        sheet.merge_range('A3:E3', 'CASH FLOW STATEMENTS', head)
        sheet.merge_range('A4:D4', '')

        if data['target_move'] == 'posted':
            sheet.write('A6', "Target Moves :", cell_format)
            sheet.write('A7', 'All Posted Entries', date)
        else:
            sheet.write('A6', "Target Moves :", cell_format)
            sheet.write('A7', 'All Entries', date)
        ...
        # sheet.merge_range('A8:F8', '', head)
        if data['levels'] == 'very':
            sheet.write('A9', 'CODE', pk)
            sheet.write('B9', 'NAME', bold)
            # sheet.write('C9', 'ACCOUNT', pk)
            sheet.write('C9', 'CASH IN (SR)', bold)
            sheet.write('D9', 'CASH OUT (SR)', bold)
            sheet.write('E9', 'BALANCE (SR)', bold)
        else:
            sheet.write('A9', 'NAME', bold)
            sheet.write('B9', 'CASH IN (SR)', bold)
            sheet.write('C9', 'CASH OUT (SR)', bold)
            sheet.write('D9', 'BALANCE (SR)', bold)
        ...
        for i in fetched_data_list:
            if data['levels'] == 'summary':
                ...
                sheet.write(row_num + 1, col_num + 1, str(i['total_debit']), amount)
                sheet.write(row_num + 1, col_num + 2, str(i['total_credit']), amount)
                sheet.write(row_num + 1, col_num + 3, str(i['total_debit'] - i['total_credit']),
                            amount)
                row_num = row_num + 1
            elif data['levels'] == 'consolidated':
                ...
                sheet.write(row_num + 1, col_num + 1, str(i['total_debit']), amount)
                sheet.write(row_num + 1, col_num + 2, str(i['total_credit']), amount)
                sheet.write(row_num + 1, col_num + 3, str(i['total_debit'] - i['total_credit']), amount)
                row_num = row_num + 1

        for j in journal_res_list:
            for k in fetched_data_list:
                if k['name'] == j['account']:
                    sheet.write(row_num + 1, col_num, str(k['name']), txt_bold)
                    sheet.write(row_num + 1, col_num + 1, str(k['total_debit']), amount_bold)
                    sheet.write(row_num + 1, col_num + 2, str(k['total_credit']), amount_bold)
                    sheet.write(row_num + 1, col_num + 3, str(k['total_debit'] - k['total_credit']), amount_bold)
                    row_num = row_num + 1
            for l in j['journal_lines']:
                sheet.write(row_num + 1, col_num, l['name'], txt_left)
                sheet.write(row_num + 1, col_num + 1, str(l['total_debit']), amount)
                sheet.write(row_num + 1, col_num + 2, str(l['total_credit']), amount)
                sheet.write(row_num + 1, col_num + 3, str(l['total_debit'] - l['total_credit']), amount)
                row_num = row_num + 1

        for j in account_res_list:
            for k in fetched_list:
                if k['name'] == j['account']:
                    sheet.write(row_num + 1, col_num, str(k['code']), pk)
                    sheet.write(row_num + 1, col_num + 1, str(k['name']), pk)
                    # sheet.write(row_num + 1, col_num + 2, str(k['name']), pk)
                    sheet.write(row_num + 1, col_num + 2, str(k['total_debit']), amount_bold)
                    sheet.write(row_num + 1, col_num + 3, str(k['total_credit']), amount_bold)
                    sheet.write(row_num + 1, col_num + 4, str(k['total_debit'] - k['total_credit']), amount_bold)
                    row_num = row_num + 1
            for l in j['journal_lines']:
                if l['account_name'] == j['account']:
                    sheet.write(row_num + 1, col_num, '', txt_left)
                    sheet.write(row_num + 1, col_num + 1, l['name'], txt_left)
                    # sheet.write(row_num + 1, col_num + 2, '', pk)
                    sheet.write(row_num + 1, col_num + 2, str(l['total_debit']), amount)
                    sheet.write(row_num + 1, col_num + 3, str(l['total_credit']), amount)
                    sheet.write(row_num + 1, col_num + 4, str(l['total_debit'] - l['total_credit']), amount)
                    row_num = row_num + 1
                for m in j['move_lines']:
                    if m['name'] == l['name']:
                        sheet.write(row_num + 1, col_num, '', txt_left)
                        sheet.write(row_num + 1, col_num + 1, m['move_name'], txt_center)
                        # sheet.write(row_num + 1, col_num + 2, '', pk)
                        sheet.write(row_num + 1, col_num + 2, str(m['total_debit']), amount)
                        sheet.write(row_num + 1, col_num + 3, str(m['total_credit']), amount)
                        sheet.write(row_num + 1, col_num + 4, str(m['total_debit'] - m['total_credit']), amount)
                        row_num = row_num + 1
        workbook.close()
        output.seek(0)
